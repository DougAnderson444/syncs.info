<script>
  ...
  import { getRootCidStr } from "../js/ipfsNode.js";
  ...
  onMount(async () => {
    ...
    if (subdomain && subdomain != "www") {
      ...
      $username = subdomain;
      let name = $page.host.includes(".localhost")
        ? subdomain + `.${DID_DOC_TLD}`
        : $page.host;
      console.log(`Getting <${name}> from HTTPS-over-DNS`);
      $dnsLink = await getDNSLinkFromName(name);
      if (!$dnsLink) status = "save-bar-container";
    }
  });

  const triggerRCS = async () => {
    $rootCidStr = await getRootCidStr($dnsLink);
  };

  $: if ($dnsLink) status = "hidden";
  $: if ($ipfsNode && $dnsLink) triggerRCS();
...
{#if status != 'hidden'}
  <div
    class={status}
    transition:fly={{ delay: 100, y: 0, duration: 2500, opacity: 0.1, easing: quintOut }}>
